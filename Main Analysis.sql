...
-- OBSERVATIONS: THE CUMULATIVE TOTALS SHOW HOW SALES AND UNITS SOLD ACCUMULATE OVER TIME, REVEALING CONSISTENT GROWTH AND HIGHLIGHTING 
-- PERIODS WHERE THE RATE OF INCREASE ACCELERATES OR SLOWS DOWN. PERSISTENT INCREASES INDICATE SUSTAINED BUSINESS GROWTH, WHILE STAGNATION 
-- OR DIPS MAY SIGNAL WEAKER PERFORMANCE.

-- RESULTS: PREVIOUS RUNS TYPICALLY SHOW STEADY CUMULATIVE GROWTH ACROSS MONTHS AND YEARS, WITH OCCASIONAL JUMPS INDICATING STRONG 
-- SALES PERIODS, AND OCCASIONAL STAGNATION OR SLOW GROWTH IN OFF-PEAK MONTHS. 
...
-- OBSERVATIONS: YEAR-OVER-YEAR PERCENT CHANGES HIGHLIGHT SALES MOMENTUM, GROWTH, OR CONTRACTION IN BOTH REVENUE AND UNITS. 
-- A POSITIVE PERCENTAGE INDICATES EXPANSION, WHILE A NEGATIVE VALUE SIGNALS DECLINE. LARGE SWINGS MAY REFLECT 
-- OPERATIONAL CHANGES, ECONOMICAL FACTORS, OR PROMOTION EFFECTIVENESS.
...
-- OBSERVATIONS: MONTHLY PERCENT CHANGES HELP TRACK SURGES OR DROPS FROM RECENT ACTIVITY. CONSISTENT INCREASES SHOW 
-- HEALTHY TRENDS, WHILE NEGATIVE MOVEMENTS MAY WARRANT TACTICAL ADJUSTMENTS. USE THIS TO EVALUATE SEASONAL PEAKS, MARKETING IMPACT, AND OPERATIONAL ISSUES.
...
-- OBSERVATIONS: PART-TO-WHOLE ANALYSIS REVEALS WHICH AREAS, ITEMS, OR CUSTOMERS MAKE UP THE LARGEST SHARE OF REVENUE, HELPING 
-- IDENTIFY CONCENTRATION RISK, TOP CONTRIBUTORS, AND OPPORTUNITIES FOR DIVERSIFICATION. USE THIS TO TARGET
-- GROWTH REGIONS, OPTIMIZE PRODUCT MIX, OR TAILOR CUSTOMER STRATEGIES.

-- 4.1 COUNTRIES --

-- THIS QUERY SUMMARIZES ORDER COUNT, SALES VALUE, AND UNITS SOLD BY COUNTRY, SHOWING EACH COUNTRY’S SALES SHARE OF THE TOTAL.

-- OBSERVATIONS: COUNTRIES WITH HIGH SALES PERCENTAGES ARE BUSINESS STRONGHOLDS. IF A FEW COUNTRIES DOMINATE, CONSIDER EXPANSION ELSEWHERE FOR BALANCE. 
-- COUNTRIES WITH LOW SALES MAY BENEFIT FROM TARGETED MARKETING OR LOCAL PARTNERSHIPS.
...
-- OBSERVATIONS: TOP-PERFORMING CITIES HIGHLIGHT GROWTH HUBS AND POTENTIAL FOR REGIONAL INVESTMENT. 
-- CITIES WITH SMALL SALES SHARES MAY REQUIRE MARKET RESEARCH OR SALES INITIATIVES. OUTLIERS CAN SURFACE BOTH RISKS AND OPPORTUNITIES.
...
-- OBSERVATIONS: PRODUCT LINES WITH HIGH SALES PERCENTAGES ARE CRITICAL. IF A FEW LINES DOMINATE, DIVERSIFY OFFERINGS OR NURTURE 
-- EMERGING PRODUCTS. LOW-SHARE LINES MAY REQUIRE PRICING REVIEWS, ENHANCEMENTS, OR DISCONTINUATION.
...
-- OBSERVATIONS: A FEW CUSTOMERS MAY DOMINATE SALES, POSING CONCENTRATION RISKS. HIGH-VALUE CUSTOMERS WARRANT RETENTION EFFORTS; 
-- LOW-SHARE CUSTOMERS MIGHT BE CANDIDATES FOR UPSELLING. REVIEW CUSTOMER MIX REGULARLY TO MAINTAIN RESILIENCE.
...
-- THIS SECTION EXPLORES ORDER PATTERNS AND PRODUCT SEGMENTS, GROUPING CUSTOMERS OR ORDERS BY,PRICE RANGE, ETC.

-- OBSERVATIONS: COHORT SEGMENTATION UNCOVERS BEHAVIOR AMONG GROUPS BASED ON RECENCY OR VALUE. IDENTIFY DORMANT USERS, LOYAL SEGMENTS, 
-- AND PRICE SENSITIVITIES. USE FINDINGS TO INFORM RETENTION STRATEGIES, TARGETED OFFERS, AND PRODUCT DEVELOPMENT.

-- 5.1 LAST ORDERS --

-- THIS ANALYSIS SEGMENTS CUSTOMERS BY TIME SINCE THEIR LAST ORDER, CATEGORIZING THEM INTO QUARTILES AND YEARLY BANDS.

-- OBSERVATIONS: MOST CUSTOMERS IN RECENT COHORTS SIGNAL STRONG ENGAGEMENT. A GROWING ‘ABOVE YEAR’ COHORT SUGGESTS ATTRITION OR INACTIVITY, 
-- WARRANTING RE-ENGAGEMENT. COMPARE DISTRIBUTIONS TO MEASURE LOYALTY AND LIFECYCLE STRENGTH.
...
-- OBSERVATIONS: THE SEGMENT DISTRIBUTION HELPS UNDERSTAND SALES CONCENTRATION IN PRICE BANDS, SHOWING CUSTOMER WILLINGNESS T
-- O PAY. A DOMINANT SEGMENT CAN INFORM PRICING STRATEGIES, PRODUCT LAUNCHES, AND PROMOTIONAL TARGETING.
...
-- OBSERVATIONS: USE THESE REPORTS TO DRILL INTO OPERATIONAL DRIVERS AT A GRANULAR LEVEL. LOCATION INSIGHTS UNCOVER REGIONAL 
-- STRENGTHS AND GROWTH OPPORTUNITIES, CUSTOMER STATS IDENTIFY ENGAGEMENT AND REVENUE CONCENTRATION, AND PRODUCT ANALYSIS DETECTS 
-- SALES LEADERS AND DISTRIBUTION TRENDS. STRATEGIC REVIEW OF THESE REPORTS HELPS OPTIMIZE RESOURCE ALLOCATION, MARKETING FOCUS, 
-- AND INVENTORY PLANNING FOR SUSTAINED GROWTH. IT COULD BE ALSO USED TO BUILD A VISUAL RREPORT IN POWER BI OR TABELAU ETC.

-- 6.1 LOCATION REPORT --

-- THIS REPORT SUMMARIZES SALES AND ORDER ACTIVITY BY COUNTRY AND CITY, PROVIDES EACH CITY’S SHARE OF ITS NATION’S MARKET, AND PREDICTS THE NEXT 
-- EXPECTED ORDER DATE.

-- OBSERVATIONS: CITIES WITH HIGH MARKET SHARE WITHIN A COUNTRY SIGNAL KEY HUBS; THOSE WITH LOWER SHARES 
-- MAY BENEFIT FROM ATTENTION OR EXPANSION. AVERAGE ORDER VALUES AND PREDICTED ORDER DATES SUPPORT SALES 
-- PLANNING AND CUSTOMER RE-ENGAGEMENT IN EACH REGION.
...
-- OBSERVATIONS: TRACK TOP CUSTOMERS TO PRIORITIZE RETENTION EFFORTS. LOW-ORDER CUSTOMERS MIGHT BE TARGETED FOR UPSELLING 
-- OR ENGAGEMENT CAMPAIGNS. REVIEWING LAST ORDER DATES SUPPORTS OUTREACH STRATEGIES AND LOYALTY INITIATIVES.
...
-- OBSERVATIONS: IDENTIFY TOP SELLERS AND PRODUCT LINES WITH HEALTHY AVERAGE ORDER VOLUMES. OUTLIERS MAY WARRANT PRODUCT DEVELOPMENT,
-- PRICING REVIEW, OR INVENTORY ADJUSTMENT. HIGH-VOLUME LINES SUGGEST MAJOR SALES CHANNELS, WHILE WEAKER LINES MIGHT NEED SUPPORT OR REEVALUATION.
